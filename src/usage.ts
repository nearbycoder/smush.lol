...
Remote sources and multipart uploads: 15 MiB. Inline base64 input and inline output: 4 MiB each, decoded. JSON/MCP HTTP request body: 6 MiB with a 30-second read deadline. Decoded images/output: 48 megapixels; output: 12000 pixels per side. Source fetches, including DNS resolution and body download, time out after 10 seconds, with at most 3 redirects. Only public HTTP(S) sources on standard ports; credentials and private-network URLs are rejected. JSON/MCP rejects unknown fields and invalid option types instead of silently accepting them.

REST failures use {"error":"message"}: 400 invalid input, 403 blocked source/origin, 408 request-body timeout, 413 size limit, 415 non-image source or wrong content type, 422 decode/transform failure, 502 remote fetch failure. MCP transport errors use JSON-RPC errors; tool failures use isError. GET and DELETE /mcp return 405 because this transport has no persistent sessions or event stream.

Files are processed in memory and not stored by the app. The hosted API/MCP sends image bytes to the server; it is not browser-only processing. AVIF encoding, interactive cropping, background removal, watermarks, redaction, finishing effects, batch ZIPs, and recipes belong to the browser editor and are not server API/MCP operations. The Image toolbox also offers local photo corrections, color replacement/keying, creative pixel effects, precise straightening, reflections, repeating patterns, and drop shadows, with PNG downloads and temporary result chaining. These tools support up to 12 megapixels / 12,000px per side including expanded layouts and are not available as API/MCP options. No animation-preservation guarantee is provided. Choose PNG or WebP when transparency is needed.
`;

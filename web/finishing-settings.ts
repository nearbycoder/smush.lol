...
export function finishingSettings(fields: Record<string, string>): Record<string, string> {
  ...
  return { trimTransparent: String(enabled(fields.trimTransparent)), trimThreshold: String(Math.round(bounded(fields, "trimThreshold", 0, 0, 254))), trimPadding: String(Math.round(bounded(fields, "trimPadding", 0, 0, 512))), pixelArt: String(enabled(fields.pixelArt)), sharpen: String(bounded(fields, "sharpen", 0, 0, 200)), contrast: String(bounded(fields, "contrast", 100, 0, 200)), exposure: String(bounded(fields, "exposure", 0, -3, 3)), colorFilter: fields.colorFilter || "none", filterAmount: String(bounded(fields, "filterAmount", 100, 0, 100)), cornerRadius: String(bounded(fields, "cornerRadius", 0, 0, 50)), borderSize: String(Math.round(bounded(fields, "borderSize", 0, 0, 1024))), borderColor: /^#[\da-f]{6}$/i.test(fields.borderColor ?? "") ? fields.borderColor! : "#ffffff", borderTransparent: String(enabled(fields.borderTransparent)) };
}
export const hasFinishing = (fields: Record<string, string>) => enabled(fields.trimTransparent) || enabled(fields.pixelArt) || hasPixelEffects(fields) || Number(fields.borderSize || 0) !== 0 || Number(fields.cornerRadius || 0) !== 0;

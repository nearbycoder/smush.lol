...
export function finishingSettings(fields: Record<string, string>): Record<string, string> {
  ...
  return { colorFilter: fields.colorFilter || "none", filterAmount: String(bounded(fields, "filterAmount", 100, 0, 100)), cornerRadius: String(bounded(fields, "cornerRadius", 0, 0, 50)), borderSize: String(Math.round(bounded(fields, "borderSize", 0, 0, 1024))), borderColor: /^#[\da-f]{6}$/i.test(fields.borderColor ?? "") ? fields.borderColor! : "#ffffff", borderTransparent: String(enabled(fields.borderTransparent)) };
}
...
export const hasPixelEffects = (fields: Record<string, string>) => Boolean(fields.colorFilter && fields.colorFilter !== "none" && Number(fields.filterAmount ?? 100) > 0);

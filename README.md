...
Homepage and API docs include static Open Graph and Twitter large-image metadata so link previews work without JavaScript. The homepage also describes the app with WebApplication structured data and three product images. Original PNGs live in `web/social/` and are copied to `public/social/` during every build; keep their public URLs and declared dimensions in sync when replacing them. Use a new filename for updated artwork to avoid stale previews cached by chat apps.
